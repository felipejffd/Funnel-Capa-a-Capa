...
router.post('/', async (req, res, next) => { try { const { name, objective, status = 'PAUSED', daily_budget, lifetime_budget, start_time, stop_time, special_ad_categories = [], is_adset_budget_sharing_enabled } = req.body; if (!name || !objective) return res.status(400).json({ error: 'name y objective son obligatorios.' }); const payload = { name, objective, status, special_ad_categories, ...(daily_budget && { daily_budget }), ...(lifetime_budget && { lifetime_budget }), ...(start_time && { start_time }), ...(stop_time && { stop_time }), ...(is_adset_budget_sharing_enabled !== undefined && { is_adset_budget_sharing_enabled }) }; const { data } = await meta.post('/' + aid() + '/campaigns', payload); res.status(201).json(data); } catch (err) { next(err); } });
